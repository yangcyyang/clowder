...
**画之前必须明确页面类型，不同类型有不同的内容约束。**

详见 [ppt-page-types.md](../refs/ppt-page-types.md)。

核心要点：
- **封面 (cover)**：只有标题+副标题+speaker/logo，❌ 禁止长段落正文
- **目录 (toc)**：短问题/章节列表（每条 ≤ 18 字），❌ 禁止长说明文字
- **章节页 (chapter)**：大标题 + 1-2 句过渡正文，❌ 禁止长段落和列表
- **内容页**：才是密度填充的主战场

**违反页面类型约束 = 结构混乱。类型是第一道门禁，密度是第二道门禁。**
...
- [ ] **页面类型正确**？（封面无长正文 / 目录无长说明 / 章节页无列表，详见 ppt-page-types.md）
...
| 错误 | 后果 | 修复 |
|------|------|------|
| 封面有长段落正文 | 结构混乱，观众认知负担增加 | 删除正文，保留标题和副标题（见 ppt-page-types.md） |
| 目录每条后面跟解释 | 信息过载，失去导航功能 | 删除解释，只保留短问题（≤18 字） |
| 章节页放了列表 | 过渡页变成内容页 | 删除列表，改成 1-2 句过渡判断 |
