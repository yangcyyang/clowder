...
## 纪律 12：根因必须落名——发现 ≠ 处置，处置必须有 owner

工作中发现**环境级/系统级根因**（例：某个自动程序在污染工作分支、某个服务配置在制造反复返工）时，禁止只在讨论里提一嘴就继续干活。当场三选一：①自己能顺手处置的，立刻处置并回报一句"已处置+怎么做的"；②自己不能处置的，建一张短标题票（守纪律 11）并明确指派/@ 到具体的猫或铲屎官，**得到"我接了"的回应才算落名**；③拿不准该不该处置的，@ 铲屎官一句话请示。判断口诀：如果现在问"这个根因谁在管？"，答不出名字=未处置。

**为什么**：2026-07-26 实证血案——推特日报晚班，main 被自动提交程序漂移 4 次、执行猫被迫两轮重做方案；衡衡早就点名"让砚砚顺手关掉"，但没人认领这个动作，一小时后 main 还在漂。**根因被三只猫都看见了，却没有一个名字挂在"关掉它"上**——发现的价值被归零。修复返工的成本远大于当场落名的十秒钟。

**工具**：`task_create`（短票+指派）/ `reply_in_thread`（@ 到人请求确认接手）/ 处置后 `post_progress` 回报。

## 纪律 13：长任务必须挂台账——人打开就能看懂的进度文件

**跨 2 场以上（定时任务多场/多次换班）或多猫协作**的工作流，owner 猫必须在频道绑定的项目目录维护一份 `progress.md` 台账，固定六节：**目标 / 当前状态 / 已完成 / 阻塞 / 下一步 / 关键裁定**（谁拍的、为什么——避免后人重新讨论已定之事）。每次收工前更新（与纪律 10"停手前清欠"同拍执行）；换班时新脑子先读台账再读交接包。**门槛写死：普通对话、单场任务禁止建台账**——状态文件只给真正的长任务，否则就是垃圾文件工厂（垃圾票的同构坑）。

**为什么**：2026-07-26 实证——铲屎官一晚三次来问"是不是卡住了/协作有没有问题"，每次都要人爬消息流拼答案：状态散在票 thread/交接包/session 摘要里，机器找得到、人找不到。正面例证同日：批次 4 执行者自发维护的 `.cat-cafe/projects/batch4/progress.md`（目标/边界/检查清单/断点），验收者和铲屎官打开即懂，零考古。
