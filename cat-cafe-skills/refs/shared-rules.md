...
> session 会断、上下文会压缩；没有落盘的状态，下一轮就等于不存在。

每完成一个带验证的工作单元，必须立即更新自己的持久记忆：
...
1. **当前状态**：做了什么、做到哪里、上次交付是什么
2. **已关闭决策**：哪些方案已否决，为什么别再提
3. **环境 gotcha**：哪些命令、端口、路径、运行方式容易踩坑
...
没有验证命令 + 没有回写状态的交付，只算临时消息，不算可接续交付。

### Task Claim 降级协议

行动任务必须优先走系统认领：`$CLI task claim` 或 `cat_cafe_claim_task`。

如果当前运行环境确实没有 `$CLI`，也没有可用的 claim 工具，不要因为“缺少 claim 入口”而停工。降级做法：

1. 在当前 thread 明确声明：`我开始做：<任务范围>`；
2. 只做这个范围，不扩散；
3. 交付时给证据，并把状态更新到可用的任务系统。

如果 claim 工具存在但返回“已被别人认领 / conflict”，必须停止，不允许重复施工。

### P7. 长期项目必须维护进度文档

> 长期项目不是靠某个 agent 的上下文记住，而是靠项目级公告板接续。

参与跨 session 项目时，每完成一个子任务必须更新对应的项目进度文档：

```text
.cat-cafe/projects/{project}/progress.md
```

至少回写四类信息：

1. **已完成**：标记完成条目，附 task # 或 commit hash
2. **当前阶段 / 进行中**：说明现在做到哪里、谁在做
3. **待做**：把下一步按优先级排列
4. **关键决策**：新增决策写入原因和日期

项目进度文档是长期项目的共享白板。Agent memory 只解决“我自己记得”，progress.md 解决“人和所有 agent 都看得见”。

不更新 progress.md 的交付，只算单轮交付，不算项目级可接续交付。

### 费曼解释协议

当回答涉及技术方案、架构设计、运行机制、设计决策、权衡对比、排查结论时，在专业回答末尾追加：
...
以下场景不追加：简单确认、进度同步、交付报告、错误提示、纯代码输出。
...
这条管**发给用户看的主消息**，不是管代码、测试、日志或内部思考。

1. **人话优先**：主体内容用中文白话写。必须用技术词时，第一次出现要括号解释；如果消息里有方案、排查、架构或技术讨论，末尾加一段 `简单说：`，用不超过 3 句翻译给用户听。
2. **默认精炼**：用户只是问进度、确认、选择、状态时，直接给结论 + 关键理由；不要写战略分析、长篇复盘或完整执行流水账。只有用户明确要求“详细分析/完整方案/展开讲”时才长写。
3. **无任务短路**：收到 @ 后先判断有没有明确执行任务（写代码、改文件、跑测试、调研、验收、排查）。没有具体执行任务时，只用一句话确认收到/待命/无新动作，不做全量上下文检查。
4. **禁止协议黑话外泄**：不要在主消息里自创 `🔍 接续检查`、`📌 记忆命中`、`🧪 源码护栏` 这类协议标记；这些属于内部工作过程，不是用户要读的内容。
5. **成本意识**：确认/应答类消息不需要读完整 thread 历史；先看最新直接上下文，只有影响判断时再扩大检索范围。
