...
description: >
  跨猫传话/交接的六项结构（What/Why/Tradeoff/Open/Next + 接球方回述确认）。
  Use when: 交接工作给其他猫、传话、写 review 信。
  Not for: 自己的任务、不涉及其他猫的工作。
  Output: 结构化交接信 + 接球方回述确认。
triggers:
  - "交接"
  - "传话"
  - "handoff"
...
**Core principle:** 交接不能只写"改了什么"。没有 Why = 接手方无法判断 = 低效协作；没有回述 = 派球方和接球方可能以为自己说的是同一件事。

## 交接六项（五件套 + 回述确认）

派球方每次交接/传话/review 请求必须包含五件套：
...
回述必须 ≤3 行。派球方确认或补料后，交接才算完成，接球方再开始执行。

### 回述豁免

一句话小任务不强制回述，例如"确认一下"、"发个路径"、"看一眼是否存在"。只要任务会跨文件、跨 session、需要交付产物、涉及 review/验收或有明确范围边界，就必须回述。
...
1. RESTATE: 先用 3 行以内回述任务、范围、缺口
2. CONFIRM: 等派球方确认；如无需补料，可开始执行
3. EXEMPT: 一句话小任务可跳过回述，但不得用于绕过复杂任务交接
```
...
```
我理解的任务是：review ADR-008 S2 Retry + CAS 的三处改动。
范围是：只看 Lua CAS 原子性、snapshotStatus 时机、竞态测试覆盖。
我还缺：信息足够。
```
