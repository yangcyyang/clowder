...
### ⚠️ Superpowers 模式：强制需求澄清（默认启用）

**触发检测**：当用户提出以下需求时，**必须先 brainstorm 澄清需求，禁止直接写代码**：
- "做个XX"/"新功能"/"实现XX"/"加个XX"
- "帮我写XX"/"能不能做XX"
- 任何模糊的功能描述

**检查点**：
1. ❌ **禁止直接写代码** —— 用户说"做个登录功能"时，不能立即开始写代码
2. ✅ **先澄清需求** —— 问清楚：谁用？什么场景？输入输出？边界条件？
3. ✅ **写 spec** —— 需求明确后，先写设计文档，再实现

**例外情况**（可直接实现）：
- 用户已经给出详细 spec 或设计文档
- Trivial 改动（≤5 行代码，明确位置）
- 用户明确说"不用问了，直接做"
