...
export function MessageActions({
  ...
  const [scheduledReminder, setScheduledReminder] = useState<AgentReminder | null>(null);
  const [isReminderBusy, setIsReminderBusy] = useState(false);
  const [reminderTargetCatId, setReminderTargetCatId] = useState('');
  const [customReminderValue, setCustomReminderValue] = useState(() => toDatetimeLocalValue(getThirtyMinutesLater()));
  ...
  const reminderExcerpt = useMemo(
    () => getMessageReminderExcerpt(message.content || '这条消息', 120),
    [message.content],
  );
  const reminderTargetOptions = useMemo(
    () =>
      cats.length > 0
        ? cats
        : reminderTargetCatId
          ? [{ id: reminderTargetCatId, displayName: reminderTargetCatId }]
          : [],
    [cats, reminderTargetCatId],
  );
  ...
  useEffect(() => {
    if (!reminderPanelOpen || reminderTargetCatId) return;
    setReminderTargetCatId(getDefaultReminderTargetCatId(message, cats));
  }, [cats, message, reminderPanelOpen, reminderTargetCatId]);
  ...
  const loadScheduledReminder = useCallback(async () => {
    ...
    try {
      ...
      const body = (await res.json().catch(() => ({}))) as { reminders?: AgentReminder[] };
      setScheduledReminder(body.reminders?.[0] ?? null);
    } catch {
      setScheduledReminder(null);
    } finally {
  ...
  const handleOpenReminderPanel = useCallback(() => {
    const defaultTarget = getDefaultReminderTargetCatId(message, cats);
    setReminderTargetCatId((current) => current || defaultTarget);
    setScheduledReminder(null);
    setCustomReminderValue(toDatetimeLocalValue(getThirtyMinutesLater()));
    setReminderPanelOpen(true);
    void loadScheduledReminder();
  }, [cats, loadScheduledReminder, message]);

  const handleScheduleReminder = useCallback(
    async (fireAt: number | null) => {
      ...
      const catId = reminderTargetCatId || getDefaultReminderTargetCatId(message, cats);
      if (!catId) {
        useToastStore.getState().addToast({
          type: 'error',
          title: '没有可提醒的 Agent',
          message: '请先配置至少一个可用成员。',
          duration: 3000,
        });
        return;
      }
      ...
      try {
        const res = await apiFetch('/api/reminders', {
          ...
          body: JSON.stringify({
            ...
            threadId,
            sourceMessageId: message.id,
            message: buildMessageReminderText(message),
            fireAt,
          }),
        });
        const body = (await res.json().catch(() => ({}))) as Partial<AgentReminder> & Record<string, unknown>;
        if (!res.ok) {
          showErrorToast('设置提醒失败', body);
          return;
        }
        const reminder = body as AgentReminder;
        setScheduledReminder(reminder);
        useToastStore.getState().addToast({
          type: 'success',
          title: '已设置稍后提醒',
          message: `将在 ${formatReminderFireTime(reminder.fireAt)} 冒泡。`,
          duration: 2600,
        });
      } catch {
        showErrorToast('设置提醒失败');
      } finally {
        setIsReminderBusy(false);
      }
    },
    [cats, message, reminderTargetCatId, threadId],
  );
  ...
  const handleCancelReminder = useCallback(async () => {
    ...
    setIsReminderBusy(true);
    try {
      const res = await apiFetch(`/api/reminders/${encodeURIComponent(scheduledReminder.id)}/cancel`, {
        method: 'POST',
      });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) {
        showErrorToast('取消提醒失败', body);
        return;
      }
      setScheduledReminder(null);
      useToastStore.getState().addToast({
        type: 'info',
        title: '已取消提醒',
        message: '这条消息不会再触发稍后提醒。',
        duration: 2200,
      });
    } catch {
      showErrorToast('取消提醒失败');
    } finally {
      setIsReminderBusy(false);
    }
  }, [scheduledReminder]);
    ...
    >
      ...
      {ctxMenu && (
        <MessageContextMenu
          ...
          onRemindLater={handleOpenReminderPanel}
      ...
      {reminderPanelOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          data-testid="message-reminder-panel"
        >
          <div className="w-full max-w-md rounded-lg border-2 border-[var(--slock-border-color)] bg-[var(--cafe-surface)] p-4 shadow-xl">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                <h3 className="text-base font-semibold text-[var(--cafe-text)]">稍后提醒</h3>
                <p className="mt-1 line-clamp-2 text-xs text-[var(--clowder-muted-soft)]">{reminderExcerpt}</p>
              </div>
              <button
                type="button"
                className="slock-message-action-button"
            ...
            <label className="mb-3 block text-xs font-semibold text-[var(--clowder-muted-soft)]">
              提醒目标
              <select
                value={reminderTargetCatId}
                onChange={(event) => setReminderTargetCatId(event.target.value)}
                className="console-form-input mt-1 h-9 w-full border-2 text-sm"
                disabled={reminderTargetOptions.length === 0 || isReminderBusy}
                aria-label="提醒目标"
              >
                {reminderTargetOptions.length === 0 ? (
                  <option value="">没有可用成员</option>
                ) : (
                  reminderTargetOptions.map((cat) => (
            ...
            {scheduledReminder ? (
              <div className="mb-3 rounded-md border border-[var(--console-border-soft)] bg-[var(--cafe-surface-elevated)] px-3 py-2 text-sm">
                <div className="font-semibold text-[var(--cafe-text)]">
                  已设置：{formatReminderFireTime(scheduledReminder.fireAt)}
                </div>
                <div className="mt-0.5 text-xs text-[var(--clowder-muted-soft)]">
                  到点后会在当前 thread 冒泡并唤醒目标。
                </div>
              </div>
            ) : null}
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                className="console-button h-9 text-sm"
                disabled={isReminderBusy || !!scheduledReminder}
                onClick={() => handleScheduleReminder(getThirtyMinutesLater())}
              >
                30分钟后
              </button>
              <button
                ...
                className="console-button h-9 text-sm"
                disabled={isReminderBusy || !!scheduledReminder}
                onClick={() => handleScheduleReminder(getTonightReminderTime())}
              >
                今晚21:00
              </button>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              <input
                type="datetime-local"
                className="console-form-input h-9 min-w-0 flex-1 border-2 text-sm"
              ...
              <button
                type="button"
                className="console-button-primary h-9 px-3 text-sm"
                disabled={isReminderBusy || !!scheduledReminder}
                onClick={() => handleScheduleReminder(parseDatetimeLocalValue(customReminderValue))}
              >
                自定义
              </button>
            </div>
            ...
            <div className="mt-4 flex justify-between gap-2">
              <button
                type="button"
                className="console-button h-9 px-3 text-sm"
                onClick={() => setReminderPanelOpen(false)}
              ...
              {scheduledReminder && (
                <button
                  type="button"
                  className="console-button h-9 px-3 text-sm text-[var(--conn-red-text)]"
                  disabled={isReminderBusy}
