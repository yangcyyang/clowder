...
export class RedisSessionChainStore implements ISessionChainStore {
  ...
  async update(id: string, patch: SessionRecordPatch): Promise<SessionRecord | null> {
    ...
    if (patch.sanityHandoff !== undefined) {
      pairs.push('sanityHandoff', JSON.stringify(patch.sanityHandoff));
    }
  ...
  private hydrate(data: Record<string, string>): SessionRecord {
    ...
    const sanityHandoff =
      data.sanityHandoff !== undefined ? safeParseJson<SessionRecord['sanityHandoff']>(data.sanityHandoff) : undefined;
