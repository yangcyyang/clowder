...
export class AgentReminderStore {
  ...
  async list(filter: { catId?: string; status?: AgentReminderStatus } = {}): Promise<AgentReminder[]> {
  ...
  async schedule(input: { catId: string; threadId: string; message: string; fireAt: number }): Promise<AgentReminder> {
    const now = Date.now();
