    ...
    try {
      ...
      await withPersonalSkillEnv(
        ...
        async () => {
          ...
          const hiddenRoute = body.skills.find((skill) => skill.name === 'hidden-route');

          assert.ok(createPrd);
          assert.ok(hiddenRoute);
          assert.equal(createPrd.source, 'personal');
          assert.equal(hiddenRoute.source, 'personal');
          assert.equal(createPrd.visible, true);
          assert.equal(hiddenRoute.visible, false);
