...
describe('GET /api/capabilities (Fastify)', () => {
  ...
  it('does not auto-add indexed personal skills as external capabilities', async () => {
    const Fastify = (await import('fastify')).default;
    const { capabilitiesRoutes } = await import('../dist/routes/capabilities.js');

    const projectDir = join('/tmp', `cap-route-test-personal-skill-${Date.now()}`);
    const homeDir = join('/tmp', `cap-route-test-personal-skill-home-${Date.now()}`);
    ...
    await writeSkill(
      ...
      `
name: create-prd
description: Create PRD from product context
`,
    );
    ...
    try {
      await withPersonalSkillEnv(
        ...
        async () => {
          ...
          assert.equal(res.statusCode, 200);
          const body = res.json();
          const personalItem = (body.items ?? []).find((item) => item.type === 'skill' && item.id === 'create-prd');
          assert.equal(personalItem, undefined, 'personal indexed skill should not appear on capabilities board');
  ...
  it('removes stale external entries that are now managed by the personal index only', async () => {
    const Fastify = (await import('fastify')).default;
    const { capabilitiesRoutes } = await import('../dist/routes/capabilities.js');

    const projectDir = join('/tmp', `cap-route-test-personal-cleanup-${Date.now()}`);
    const homeDir = join('/tmp', `cap-route-test-personal-cleanup-home-${Date.now()}`);
    const personalRoot = join(homeDir, '.claude', 'skills');
    const indexPath = join(projectDir, '.cat-cafe', 'personal-skills-index.json');
    const prevHome = process.env.HOME;
      ...
      await withPersonalSkillEnv(
        ...
        async () => {
          ...
          assert.equal(
            ...
            false,
            'personal indexed external skill should be hidden from capabilities board',
          );
          assert.equal(
            (body.items ?? []).some((item) => item.type === 'skill' && item.id === 'unrelated-external'),
            true,
            'unrelated external skill should remain visible',
          );
  ...
  it('keeps CapabilityBoardItem source limited to cat-cafe and external', async () => {
    const source = await readFile(join(process.cwd(), 'packages/shared/src/types/capability.ts'), 'utf-8');
    assert.match(source, /source:\s*'cat-cafe'\s*\|\s*'external';/);
    assert.doesNotMatch(source, /source:\s*'cat-cafe'\s*\|\s*'external'\s*\|\s*'personal'/);
  });
