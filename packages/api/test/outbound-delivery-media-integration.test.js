...
describe('OutboundDeliveryHook — media delivery integration', () => {
  ...
  // W5d: the voice chain (TTS/VoiceBlockSynthesizer) was removed, so there is no
  // more resolver to inject here — audio{text, no url} always degrades to text.

  it('degrades audio{text, no url} to text — no silent loss', async () => {
    const { OutboundDeliveryHook } = await import('../dist/infrastructure/connectors/OutboundDeliveryHook.js');
