...
> **状态（2026-07-18 更新）**：下文 2026-07-14 的“`default` + 窄 allow”是当时的最小权限修复记录，已被 [ADR-025](../../decisions/025-grok-trusted-headless-permissions.md) 的显式产品决策取代。当前 Grok trusted headless profile 使用 `bypassPermissions`；这不是为了让测试通过的临时绕过。
...
| **6. 预警策略（历史）** | 2026-07-14 时，若需要 `bypassPermissions` / `--always-approve` 才能通过，或未 allow 的其他工具也被自动批准，即判定偏离当时的最小权限方案。该预警已由 ADR-025 的 trusted profile 风险边界替代。 |
| **7. 用户可见交互修正** | 权限取消时明确显示“终端工具未通过权限确认，本轮未完成”，不再把半截承诺当成成功回复。 |
| **8. 验收（历史）** | 2026-07-14 的 RED/GREEN 曾锁定 `permission-mode=default` + `--allow Bash`，并验证 `Cancelled` 在 partial text 之后产生明确 error。权限模式的当前验收以 ADR-025 和 `security-boundary.test.js` 为准。 |
...
真实负向复验还发现：旧参数 `permission-mode=auto` 会让未显式 allow 的 `Write` 直接落盘，不能证明“未来新工具仍走门”。因此在 2026-07-14 的阶段性修复中，基线改为 `default`，再以独立 allow 精确放行 MCP namespace 与 `Bash`。该推论仅适用于当时的窄权限 profile，已在 2026-07-18 被 ADR-025 的 trusted profile 决策替代。
...
1. **历史修复（2026-07-14）**：将 permission mode 从 `auto` 收紧为 `default`，在现有 MCP allow 之外追加独立的 `--allow Bash`，仅放行终端工具类型；当前权限模式见 ADR-025。
...
历史上放弃的备选是 `bypassPermissions`、`--always-approve`、将所有工具统一 allow；它们超出 2026-07-14 的最小权限边界。2026-07-18 铲屎官已明确选择 trusted headless profile（见 ADR-025），其中 `bypassPermissions` 是被接受且有边界说明的策略，不再作为本报告的未决备选。

## 验证方式

- 历史参数单测：曾验证 permission mode 为 `default`，同时存在 MCP namespace 与 `Bash` 两条 allow，且不存在 blanket bypass；当前边界由 `security-boundary.test.js` 锁定为 ADR-025 的 trusted profile。
...
当时的修复保持 `permission-mode=default`，仅在 MCP namespace 与 `Bash` 之外显式追加 `Write`、`Edit`；当时仍禁止 `--always-approve`、`bypassPermissions` 和全工具放行。这个历史边界没有新增 Bash 做不到的文件写能力，但让文件变更走可审计的结构化工具，避免为绕过权限门而退化成 shell 写文件。当前策略见 ADR-025。
...
## 2026-07-18 决策变更：trusted headless profile

后续真实任务表明，`default` 加窄 allow 会继续把无头工程回合卡在权限确认上；铲屎官随后明确要求 Grok 权限“改到跟 Claude 一样”，并授权使用与 Claude provider 对齐的 `bypassPermissions`。实现提交为 `d7c13201`。

因此，当前 Grok profile 采用 `bypassPermissions`，并保留 Cat Cafe MCP、`Bash`、`Write`、`Edit` 四条 `--allow` 作为**能力文档，而不是安全边界**。真正仍有效的边界是：每次调用创建隔离的 `GROK_HOME`、只注册 Cat Cafe MCP、桥接进程使用最小环境变量、Cat Cafe callback 的身份/工作区权限校验，以及 Grok 的 deny rules、hooks 与管理员锁（如有配置）。

残余风险被明确接受：Grok 内置工具会在该 trusted profile 中普遍自动批准；宿主没有 OS 级沙箱，内置 `Bash`/`Write`/`Edit` 以运行服务的同一 OS 身份执行。任何改回窄权限或扩大运行范围的提议，都必须重新取得铲屎官决定并以真实 smoke 复验。安全边界测试同时检查运行时 MCP 隔离、机密不写入配置/桥接文件，以及 ADR 记录存在。
