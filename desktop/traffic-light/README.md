...
- `Clowder`：读取 `GET /api/runtime/traffic-light`。
  ...
  - 灰灯：无法连接 Clowder API。
- `Slock`：读取本机 Slock daemon lock，并扫描本机 runtime / bridge 进程。
  - 黄灯：检测到 Slock-launched runtime CPU 活跃；CPU 短暂掉到 0 时会保持约 8 秒防抖。
  - 绿灯：daemon / bridge / runtime 在线，但持续未检测到活跃执行。
  - 灰灯：daemon 未运行。
  - 注意：这是本机侧近似判断，不读取 Slock 服务端内部队列。
- `Codex`：扫描本机 `opencode` / `codex` 客户端、`codex app-server` 和 `--runtime codex` 进程。
  - 黄灯：检测到 Codex / app-server / runtime CPU 活跃；CPU 短暂掉到 0 时会保持约 8 秒防抖。
  - 绿灯：客户端 / bridge / runtime 在线，但持续未检测到活跃执行。
  - 灰灯：未检测到客户端进程。
  - 注意：`chat-bridge.js` 和空闲 `codex app-server` 属于常驻在线信号，不单独视为运行中。
...
启动后会出现一个约 `420×56` 的置顶状态条。点击状态条会打开 `http://127.0.0.1:3003`。
